-- Bootstrap busted to run the zcmp suite inside Neovim via `nvim -l`.
--
-- The suite exercises real Neovim APIs (options, buffers, keymaps, vim.lsp,
-- ...), so it must run inside Neovim rather than standalone Lua.
...
-- vim.lsp is lazily loaded on first access, and which spec triggers that load
-- depends on the order they were collected in. Loading it up front makes the
-- ordering irrelevant.
